...
- Reads your private key from env variable `EVM_PRIVATE_KEY`
...
- Registers both V1 and V2 scheme clients for EVM (Etherlink)
...
- Selects a supported token (e.g. BBT on Etherlink)
- Signs a Permit2 authorization (EVM)
...
# Prerequisites
- Private key with Etherlink funds for BBT payments
- Rust + Cargo
- `EVM_PRIVATE_KEY` set in your environment (or in `.env` file, see `.env.example`)
...
# 2. Create `.env` file
cp .env.example .env
# 3. Set your EVM_PRIVATE_KEY inside `.env`
# 4. Run
cargo run
```
